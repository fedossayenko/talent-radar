...
@ApiTags('scraper-v2')
@Controller('scraper/v2')
export class ScraperControllerV2 {
  private readonly logger = new Logger(ScraperControllerV2.name);

  constructor(
    private readonly scraperServiceV2: ScraperServiceV2,
  ...
  async scrapeMultipleSites(
    ...
    try {
      const result = await this.scraperServiceV2.scrapeAllSites({
        sites: sitesArray,
        limit: limitNum,
  ...
  async scrapeSingleSite(
    ...
    try {
      const result = await this.scraperServiceV2.scrapeSite(siteName, {
        limit: limitNum,
        enableAiExtraction: enableAIFlag,
        enableCompanyAnalysis: enableCompanyAnalysisFlag,
        force: forceFlag,
  ...
  async getEnhancedStats() {
    ...
    try {
      const [scrapingStats, scraperStats] = await Promise.all([
        this.scraperServiceV2.getScrapingStats(),
        this.scraperServiceV2.getScraperStats(),
      ]);
      ...
      return {
        ...
        data: {
          ...
          availableScrapers: this.scraperServiceV2.getAvailableScrapers(),
        },
      };
    } catch (error) {
  ...
  async getAvailableScrapers() {
    ...
    try {
      const scrapers = this.scraperServiceV2.getAvailableScrapers();
      const stats = this.scraperServiceV2.getScraperStats();
  ...
  async legacyTestDevBgScraping(
    ...
    try {
      const result = await this.scraperServiceV2.scrapeSite('dev.bg', {
        limit: limitNum,
